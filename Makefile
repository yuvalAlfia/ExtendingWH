...
bin : libwh.so
libwh.so : Makefile Makefile.common lib.c lib.h kv.c kv.h wh.c wh.h wh.strip hash.c hash.h
	$(eval ALLFLG := $(CSTD) $(EXTRA) $(FLG) -shared -fPIC)
	$(eval ALLLIB := $(addprefix -l,$(LIB) $(LIB-$@)))
	$(CCC) $(ALLFLG) -o $@ lib.c kv.c wh.c hash.c $(ALLLIB)
