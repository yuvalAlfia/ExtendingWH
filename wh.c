    ...
    const void * const vbuf, const u32 vlen, hash_table const *hash)
{
  ...
  int kToInt = atoi(kbuf);
  int timeToInt = (int) currTime;
  int* newVal = (int*)malloc(sizeof(int)*2);
  newVal[0] = timeToInt;
  newVal[1] = flag;
  printf("insert key = %d with time = %d\n", kToInt, timeToInt);
  hash_insert(hash,kToInt,newVal);
...
wh_probe(struct wormref * const ref, const void * const kbuf, const u32 klen, hash_table const *hash)
{
  struct kref kref;
  kref_ref_hash32(&kref, kbuf, klen); 
  bool res = wh_api->probe(ref, &kref);
