    ...
    const void * const value, const u32 vlen)
{
  ...
  kv_refill_value(kv, value, vlen); //config vlen
  kv_update_hash(kv);
}
    ...
    const void * const value, const u32 vlen)
{
  ...
  inline struct kref
kv_kref(const struct kv * const key)//creates kref
{
  return (struct kref){.ptr = key->kv, .len = key->klen, .hash32 = key->hashlo};
}

  inline struct kv * //creates key value and put it in hash
kv_create(const void * const key, const u32 klen, const void * const value, const u32 vlen)
{
  ...
  if (kv)
    kv_refill(kv, key, klen, value, vlen); //put it in hash
  return kv;
}
...
__attribute__((constructor)) //create empty kv and put in hash
  static void
kv_null_init(void)
{
  ...
  inline const struct kv *   //return null kv
kv_null(void)
{
  return &__kv_null;
}
...
// dup {{{
  inline struct kv *
kv_dup(const struct kv * const kv)  //duplicates kv
{
  if (kv == NULL)
    return NULL;
...
kref_ref_raw(struct kref * const kref, const u8 * const ptr, const u32 len)
{
  ...
  inline void
kref_ref_hash32(struct kref * const kref, const u8 * const ptr, const u32 len) //creates ref
{
  kref->ptr = ptr;
  kref->len = len;
